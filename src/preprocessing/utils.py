...
def load_hydrobasins(hydro_basin_csv):
    ...
    return gdf
def validate_hybas_uparea_consistency(
    hydroatlas_csv,
    hydrobasin_csv,
    area_tol=0.01,   # 1% relative difference
    verbose=True
):
    """
    Validate consistency of UP_AREA between
    HydroATLAS and HydroBASINS.

    Parameters
    ----------
    hydroatlas_csv : str
        Path to HydroATLAS CSV
    hydrobasin_csv : str
        Path to aggregated HydroBASINS CSV
    area_tol : float
        Relative difference tolerance (e.g. 0.01 = 1%)
    """

    print("[VALIDATE] Load HydroATLAS...")
    atlas_gdf = load_hydroatlas(hydroatlas_csv)
    atlas_df = atlas_gdf[["hybas_id", "up_area"]].copy()

    print("[VALIDATE] Load HydroBASINS...")
    basin_gdf = load_hydrobasins(hydrobasin_csv)
    basin_df = basin_gdf[["hybas_id", "up_area"]].copy()

    # ==================================================
    # 1. HYBAS existence check
    # ==================================================
    atlas_ids = set(atlas_df.hybas_id)
    basin_ids = set(basin_df.hybas_id)

    missing_in_atlas = basin_ids - atlas_ids
    missing_in_basin = atlas_ids - basin_ids

    # ==================================================
    # 2. UP_AREA comparison
    # ==================================================
    merged = basin_df.merge(
        atlas_df,
        on="hybas_id",
        how="inner",
        suffixes=("_basin", "_atlas")
    )

    # remove invalid
    merged = merged[
        (merged.up_area_basin > 0) &
        (merged.up_area_atlas > 0)
    ]

    merged["rel_diff"] = (
        (merged.up_area_atlas - merged.up_area_basin).abs()
        / merged.up_area_basin
    )

    area_mismatch = merged[merged.rel_diff > area_tol]

    # ==================================================
    # 3. Reporting
    # ==================================================
    print("\n================ UP_AREA VALIDATION ================")
    print(f"Total HydroBASINS basins : {len(basin_df)}")
    print(f"Total HydroATLAS basins  : {len(atlas_df)}")

    print(f"\nHYBAS missing in ATLAS   : {len(missing_in_atlas)}")
    print(f"HYBAS missing in BASINS : {len(missing_in_basin)}")

    print(
        f"\nUP_AREA mismatch (> {area_tol*100:.1f}%): "
        f"{len(area_mismatch)}"
    )

    if verbose and len(area_mismatch) > 0:
        print("\n[Top UP_AREA mismatches]")
        print(
            area_mismatch
            .sort_values("rel_diff", ascending=False)
            .head(10)[
                ["hybas_id", "up_area_basin", "up_area_atlas", "rel_diff"]
            ]
        )
    ...
    return {
        "missing_in_atlas": pd.DataFrame({"hybas_id": list(missing_in_atlas)}),
        "missing_in_basin": pd.DataFrame({"hybas_id": list(missing_in_basin)}),
        "uparea_mismatch": area_mismatch
    }
...
#Plot
def plot_coverage_station_over_area(
    df_or_path,
    area_col="sub_area",
    grdc_col="grdc_no",
    rain_col="rain_station_id",
    min_area=None,
    log_area=True
):
    """
    Plot number of rain stations vs upstream area per GRDC.
    ...
    # --------------------------------------------------
    area_df = (
        df.groupby(grdc_col)[area_col]
          .sum()
          .rename("upstream_area")
    )

    rain_cnt_df = (
        df[df[rain_col].notna()]
        .groupby(grdc_col)[rain_col]
        .nunique()
        .rename("n_rain_station")
    )
    ...
    if min_area is not None:
        cov_df = cov_df[cov_df.upstream_area >= min_area]

    x = cov_df["upstream_area"].values
    y = cov_df["n_rain_station"].values
    ...
    if len(x) >= 2:
        if log_area:
            ...
            plt.plot(xs, z[0] * np.log1p(xs) + z[1], linestyle="--")
        else:
            z = np.polyfit(x, y, 1)
            xs = np.linspace(x.min(), x.max(), 200)
            plt.plot(xs, z[0] * xs + z[1], linestyle="--")

    plt.xlabel("Upstream area (km²)")
    plt.ylabel("Number of rain stations")
    plt.title("Rain station coverage vs upstream area")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
def compute_grdc_upstream_rain_coverage(
    step1_upstream_csv: str,
    step2_rain_map_csv: str,
):
    """
    Compute upstream rain coverage per GRDC.
    ...
    total_area_df = (
        ...
        .sum()
        .rename("total_upstream_area")
        .reset_index()
    )

    # --- load STEP 2 (ONLY basins with rain stations)
    step2 = pd.read_csv(step2_rain_map_csv)
    ...
    # --- merge
    cov_df = total_area_df.merge(
        covered_area_df,
        on="grdc_no",
        how="left"
    )

    cov_df["covered_area"] = cov_df["covered_area"].fillna(0.0)
    cov_df["coverage_ratio"] = (
        cov_df["covered_area"] / cov_df["total_upstream_area"]
    )

    return cov_df
def plot_grdc_upstream_rain_coverage(
    coverage_df,
    bins=20
):
    """
    Visualize upstream rain coverage per GRDC.

    Plots:
    1) Histogram of coverage ratio
    2) Scatter: total upstream area vs coverage
    """
    ...
    plt.xlabel("Upstream rain coverage ratio")
    plt.ylabel("Number of GRDC stations")
    plt.title("Distribution of upstream rain coverage")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    # --------------------------------------------------
    # Scatter: area vs coverage
    # --------------------------------------------------
    plt.figure(figsize=(7, 4))

    x = coverage_df["total_upstream_area"].values
    y = coverage_df["coverage_ratio"].values
    ...
    plt.xscale("log")
    plt.xlabel("Total upstream area (km², log scale)")
    plt.ylabel("Upstream rain coverage ratio")
    plt.title("Rain coverage vs upstream area")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
